    ...
    performBasicSearch() {
      ...
      this.searchResults = this.allData.filter(item => {
        return (
          ...
          (item.old_dialect_phonetic && item.old_dialect_phonetic.toLowerCase().includes(searchTerm)) ||
          (item.new_dialect_phonetic && item.new_dialect_phonetic.toLowerCase().includes(searchTerm))
        );
      });
    },
